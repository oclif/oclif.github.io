...
If you want to see the output but leave it mocked, you can either pass in `{print: true}` to the options, or set `DEBUG=*` (to see all output) or `DEBUG=stdout|stderr` (to see just the stdout/stderr).
